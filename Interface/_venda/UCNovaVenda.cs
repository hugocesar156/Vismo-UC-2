    ...
    public partial class UCNovaVenda : UserControl
    {
        ...
        private void DgvLista_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                ...
                for (int i = 0; i < dgvPesquisa.RowCount; i++)
                {
                    if (Convert.ToString(dgvLista.CurrentRow.Cells[0].Value) == Convert.ToString(dgvPesquisa.Rows[i].Cells[0].Value))
                    {
                        int aux1 = Convert.ToInt32(dgvLista.CurrentRow.Cells[2].Value);
                        int aux2 = Convert.ToInt32(dgvPesquisa.Rows[i].Cells[2].Value);

                        aux1 += aux2;

                        dgvPesquisa.Rows[i].Cells[2].Value = aux1;
                    }
                }
